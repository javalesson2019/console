		...
		while (true) {
			...
			if (matcher.find()) {
				...
				if (props.containsKey(match)) {
					...
					try {
						propClass = Class.forName(className); // TODO: это очень дорого при каждом вводе вытягивать
						                                      // через рефлексию. Надо было один раз загрузить классы
						                                      // и многократно их повторно использовать
